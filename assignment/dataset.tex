...
CLOTH\upcite{CLOTH}数据集是收集自中国中学生的英语考试试题中的完型填空题型。与之前的填空型数据集最大的不同是问题中删除的单词不像之前数据集的构建方式那样是由系统根据规则自动构建的，这样生成的问题往往没有目的性并且答案（也就是缺失的单词）可能会出现歧义。
...
接近真实世界的语义。另外RACE数据集中文章主题的覆盖度比其它的数据集更广泛，比如CNN\&Daily\upcite{CNNDailyMail}
...
单词的个数。片段选择型问答任务数据集较其它类型任务的数据集较多，常用的数据集如SQuAD 1.1\upcite{SQuAD1}、
...
是没有答案的，这也使得后来Rajpurkar等人在SQuAD 1.1版本上又增加了五万个不可回答的问题构建了数据集SQuAD 2.0\upcite{SQuAD2}。对于这些带有不可回答问题的数据集，模型首先要清楚问题是否可以根据文章回答，在可回答的前提下给出答案，因此要求模型对文章的理解要更加的深刻。
...
Yang等人\upcite{HotpotQA}发布了HotpotQA数据集，每一个问题对应多个段落，问题的答案往往需要在多个段落上逐步推理才能获得。
...
ROUGE-L\upcite{Rouge}和BLEU\upcite{BLEU}。ROUGE-L用来计算标准答案和预测答案的最长公共子序列（Longest Common Subsequence，LCS），BLEU用来衡量预测答案和真实答案之间的相似性。
...
\begin{table}[ht]
	\centering
	\caption{MRC常用数据集对比 \\ Table 5 Comparison of common dataset in MRC}
	%\vspace{5pt}
	\resizebox{\textwidth}{!}{
		\begin{tabular}{l c c p{6cm}<{\centering} c}
			\toprule
			数据集&文章来源/大小&文章类型&问题类型&答案类型 \\
			\midrule
			CNN\&Daily Mail\upcite{CNNDailyMail}&新闻文章/1.4M&单段落型&删除句子中某个命名实体构成问题&填空型 \\
			\midrule
			CBT\upcite{CBT}&儿童故事书/688K&单段落型&删除句子中某个单词构成问题&填空型 \\
			\midrule
			CLOTH\upcite{CLOTH}&英语考试/99K&单段落型&教师基于文章生成填空型问题&填空型 \\
			\midrule
			MCTest\upcite{MCTest}&儿童故事/2.6K&单段落型&人工基于文章生成问题&多项选择型 \\
			\midrule
			RACE\upcite{RACE}&英语考试/100K&单段落型&教师基于文章生成问题&多项选择型\\
			\midrule
			SQuAD 1.1\upcite{SQuAD1}&维基百科/100K&单段落型&人工基于文章生成问题&片段选择型\\
			\midrule
			TriviaQA\upcite{TriviaQA}&网页搜索/650K&多段落型&收集于网站上的问题-答案对&片段选择型\\
			\midrule
			SQuAD 2.0\upcite{SQuAD2}&维基百科/100K&单段落型&新增不可回答问题&片段选择型\\
			\midrule
			NewsQA\upcite{NewsQA}&新闻文章/120K&单段落型&人工基于文章生成&片段选择型\\
			\midrule
			HotpotQA\upcite{HotpotQA}&维基百科/113K&多段落型&人工基于多篇文章生成问题，答案的查找需多步推理&片段选择型\\
			\midrule
			MS MARCO\upcite{MSmarco}&搜素引擎/100K&多段落型&收集于必应搜索用户的问题&自由答案型\\
			\midrule
